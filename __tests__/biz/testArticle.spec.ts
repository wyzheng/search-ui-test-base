...
import {bizOperation, getHeightOfEle, getHighlightContent, superView} from "../../lib/utils/tools";
import fs from "fs";
  ...
  test("testBIzBoxHighlight", async () => {
    ...
    while (num != 0) {
      try {
        ...
        const notMatchingElements = articleEle.filter(
          (el) => !highlightEle.includes(el)
        );
        const querySet = new Set("爹味漫长的季节".split(''));
        for (const element of notMatchingElements) {
          // 在这里对每个元素进行操作，获取元素的html文本
          const text = await page.evaluate((el) => el.textContent, element);
          console.log(`Element text: ${text}`);
          const eleSet = new Set(text.split(''));
          const intersection = new Set([...querySet].filter(x => eleSet.has(x)));
          expect(Array.from(intersection).join('') == '')
        }
        break;
      } catch (e) {
        if (num == 1) {
